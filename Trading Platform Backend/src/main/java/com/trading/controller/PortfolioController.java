...
public class PortfolioController {
    ...
    @Operation(summary = "Get user portfolio", description = "Retrieve all stock holdings for a user with profit/loss calculations")
    @GetMapping("/{username}")
    ...
    @Operation(summary = "Get portfolio value", description = "Calculate total value of all holdings")
    ...
    @Operation(summary = "Get transaction history", description = "Retrieve all buy/sell transactions for a user")
    ...
    @Operation(summary = "Get realized profit/loss", description = "Calculate total realized P&L from all completed buy/sell transactions")
    @GetMapping("/{username}/realized-pnl")
    public ResponseEntity<ApiResponse> getRealizedProfitLoss(@PathVariable String username) {
        try {
            BigDecimal realizedPnl = portfolioService.getRealizedProfitLoss(username);
            return ResponseEntity.ok(new ApiResponse(true, "Realized P&L calculated", realizedPnl));
        } catch (Exception e) {
            return ResponseEntity.badRequest()
                    .body(new ApiResponse(false, e.getMessage()));
        }
